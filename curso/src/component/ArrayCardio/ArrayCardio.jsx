...
const ArrayCardio = () => {
  ...
  const filterInThe1500s = inventors.filter((inventor) => inventor.year >= 1500 && inventor.year < 1600);
  // console.table(filterInThe1500s);

  // Array.prototype.map()
  // 2. Give us an array of the inventors' first and last names
  const mapInventorsName = inventors.map((inventor) => {return {first: inventor.first, last:inventor.last}});
  // console.table(mapInventorsName);
  ...
  const yearsLived = inventors.sort((firstPerson, secondPerson) => {
    const firstPersonLived  = firstPerson.passed - firstPerson.year; 
    const secondPersonLived = secondPerson.passed - secondPerson.year;
    return firstPersonLived > secondPersonLived ? 1 : -1;
  })
  ...
  // const list = Array.from(document.querySelectorAll('.mw-category-group ul li a'));
  // const de = list
  //                 .map(link => link.title)
  ...
  const alphabetically = people.sort((firstPerson, secondPerson) => {
    const [nameFP, lastNameFP]  = firstPerson.split(', ');
    const [nameSP, lastNameSP] = secondPerson.split(', ');

    return lastNameFP > lastNameSP ? 1 : -1
  });
  ...
  return (
    <div>
      <h1>Hi!! Welcome to Array Cardio Day 1</h1>
      <span>Go to the inspector to see magic ;D</span>
    </div>
  )
}
